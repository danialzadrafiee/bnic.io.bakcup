            ...
            <form method="dialog" class="w-full max-w-md modal-box p-0 ">
                ...
                <div class="">
                    ...
                    <div class="p-6 space-y-6 flex flex-col items-center">
                        {{-- NFT Box --}}
                        <nft
                            class="js-nft-box w-[300px] h-[320px] border bg-white flex items-center gap-2.5 py-4 px-0 rounded border-neutral/50 flex-col ">
                            <code>
                                @if ($user->user_type == 'invidual')
                                    <span class="uppercase">
                                        {{ $user->gender[0] }}-{{ substr(hash('sha256', $user->email), 0, 8) }}-{{ $user->id }}-<span
                                            contenteditable class="js-span-token">{{ $user->token }}</span>
                                    </span>
                                @else
                                    <span class="uppercase">
                                        {{ $user->corp_type[0] }}-{{ substr(hash('sha256', $user->email), 0, 8) }}-{{ $user->id }}-<span
                                            contenteditable class="js-span-token">{{ $user->token }}</span>
                                    </span>
                                @endif
                            </code>
                            <qrcode class="flex items-center justify-center">
                                ...
                                <div class="js-nft-qrcode !h-[220px]">
                                    ...
                                    <x-qrcode data="{{ route('dashboard.public_index', [$user->id]) }}"></x-qrcode>
                                </div>
                                <design class="bg-base-content block w-[46px] h-[60px]">
                                </design>
                            </qrcode>
                    ...
                    <!-- Modal footer -->
                    <div
                        class="flex items-center justify-center p-6 space-x-2 border-t border-neutral-200 rounded-b">

                        <button for="sign_wallet_modal" class="btn btn-neutral">Decline</button>
                        <button data-id={{ $user->id }} type="button"
                            class="js-btn-signchain-submit btn btn-primary">Generate
                            Profile NFT</button>
                    </div>
                </div>
            </form>
