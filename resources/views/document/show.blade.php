    ...
    <main class="flex">
        <core class="w-full">
            ...
            <article class="w-full">
                ...
                <section class="bg-white rounded-xl shadow-lg mt-8 py-12">


                    </header>
                    <form method="POST" class="js-form-data bg-white mx-auto p-12">
                        <header>
                            <categories class="flex py-4 px-8 gap-4 items-center">
                                @if (App\Models\User::find($document->corporation->id)->corp_cat_pri)
                                    <card class="border rounded-xl p-4">
                                        {{ App\Models\User::find($document->corporation->id)->corp_cat_pri }}</card>
                                @endif
                                @if (App\Models\User::find($document->corporation->id)->corp_cat_sec)
                                    <card class="border rounded-xl p-4">
                                        {{ App\Models\User::find($document->corporation->id)->corp_cat_sec }}</card>
                                @endif
                            </categories>
                            {!! $document->content !!}
                    </form>
                    <form action="{{ route('cert.sign') }}" method="POST" class="js-form-real block w-full">
                        @if (auth()->user()->id != $document->corporation->id)
                            ...
                            @csrf
                            <additional class="px-8 w-full block">
                                <label>Additional people </label>
                                <row
                                    class="js-row-additional grid grid-flow-col gap-2 py-2 justify-stretch w-full grid-cols-3 ">
                        ...
                        @endif
                        @if (auth()->user()->id != $document->corporation->id)
                            <button type="button" class="js-btn-sign btn btn-primary mx-8 my-8">
                                Sign Document
                            </button>
                        @else
        ...
        <modal
            ...
            style="display:none">
            <inside class="w-max rounded flex-col flex bg-white">
                <header class="flex items-center p-4 justify-between">
                    ...
                    <searchbox class="flex">
                        <input type="text"
                            class="js-input-search-user bg-neutral/5 rounded rounded-r-none border border-neutral/20 px-4 py-2"
                ...
                <section class="js-section-search-result p-4 overflow-y-auto h-[500px]">
                    @foreach ($modal_users as $user)
                        <user class="js-row-user rounded border-b py-4  border-neutral/20 flex gap-4 flex-row w-full  ">
                            ...
                            <column class="flex flex-col hidden  self-center">
                            ...
                            <column class="self-start items-center">
                                <button class="js-btn-select-user btn btn-primary btn-sm"
