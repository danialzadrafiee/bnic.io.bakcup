<x-layout.dashboard :user="$cert->corporation">
    @vite(['resources/css/corporation-show.scss', 'resources/js/cert-show.js'])
    @php
        $blockchainised =
    ...
    <main class="flex">
        <article>
            ...
            <section class="js-cert-node bg-white py-20 px-5 rounded-lg">

                <form class="bg-white max-w-full mx-auto relative">
                    @if (request()->has('success'))
                        <success
                            class="absolute w-full rounded py-4 flex top-[-55px] mx-auto items-center justify-center bg-[#c6ffd0] text-green-900">
                            <close class="absolute top-0 cursor-pointer right-2"
                    ...
                    @endif
                    <header class="px-2 flex items-end justify-between">
                        <start class="flex text-sm justify-end items-end  gap-2">
                            <qr class="w-24 ">
                                <x-qrcode data="{{ route('cert.show', ['id' => $cert->id]) }}"></x-qrcode>
                            </qr>
                            <column class="flex flex-col -mb-1">
                                <flex class="flex gap-2 items-center">
                                    <name class="text-base font-semibold ">{{ $cert->name }}
                                    </name>
                                    <token contenteditable class="js-cert-token text-xs">{{ $cert->token }}</token>
                                </flex>
                                ...
                                <tooltip class="tooltip js-cert-modal-tooltip"
                                    ...
                                    <button type="button" data-reciver="{{ $cert->reciver_verify }}"
                                        ...
                                        <badges class="flex gap-2 items-center">
                                            <div
                                                class="badge {{ $cert->creator_verify == 0 ? 'badge-neutral' : 'badge-success' }} badge-sm  gap-2">
                                                creator
                                            </div>
                                            <div
                                                class="badge {{ $cert->reciver_verify == 0 ? 'badge-neutral' : 'badge-success' }}  badge-sm  gap-2">
                                                reciver
                                            </div>
                                        </badges>
                                    </button>
                ...
                <form action="{{ route('cert.sign') }}" method="POST" class="js-form-real">
                    ...
                    <additional>
                        ...
                        @foreach (json_decode($cert->ad_email) as $email)
                            <row class="js-row-additional">
                                <input placeholder="Email" name="ad_email[]" value="{{ $email }}">
                                <input placeholder="Role" name="ad_role[]"
                    ...
                    <signatures class="grid grid-cols-3 gap-4 w-full mt-8 pt-4 border-t border-neutral/50">
                        ...
                        @if ($cert->reciver_verify == 0)
                            <column class="col-span-1">
                                <card class="block max-w-sm p-6 bg-white border border-black/20 rounded-lg shadow  ">
                                    <h5 class="mb-2 text-lg font-semibold tracking-tight text-black/90 ">
                                        Reciver</h5>
                                    <p
                                        class="border rounded w-full aspect-square flex items-center justify-center font-semibold">
                                        Pending
                                    </p>
                                </card>
                            </column>
                        ...
                        @if ($cert->reciver_verify == 1)
                            @php
                                $profile_qrcode = str_replace('.json', '.png', App\Models\User::where('email', $cert->reciver)->first()->profile_nft_id);
                            @endphp
                            <column class="col-span-1">
                                <card class="block max-w-sm p-6 bg-white border border-black/20 rounded-lg shadow  ">
                                    <h5 class="mb-2 text-lg font-semibold tracking-tight text-black/90 ">
                                        Reciver</h5>
                                    <p
                                        class="{{ $profile_qrcode ? '' : 'border' }} rounded w-full aspect-square flex items-center justify-center font-semibold">
                                        @if ($profile_qrcode)
                                            <img src="{{ $profile_qrcode }}" class="w-full h-full">
                                        @else
                                            Pending
                                        @endif
                                    </p>
                        ...
                        @if ($cert->creator_verify == 1)
                            @php
                                $profile_qrcode = str_replace('.json', '.png', App\Models\User::where('id', $cert->corporation_id)->first()->profile_nft_id);
                            @endphp
                            <column class="col-span-1">
                                <card class="block max-w-sm p-6 bg-white border border-black/20 rounded-lg shadow  ">
                                    <h5 class="mb-2 text-lg font-semibold tracking-tight text-black/90 ">
                        ...
                        @php
                            ...
                            $profile_qrcode_id ? ($profile_qrcode_json = str_replace('.json', '.png', App\Models\Nft::where('id', $profile_qrcode_id)->first()->url)) : ($rofile_qrcode_json = null);
                        @endphp
                        <column class="col-span-1">
                            <card class="block max-w-sm p-6 bg-white border border-black/20 rounded-lg shadow  ">
                                <h5 class="mb-2 text-lg font-semibold tracking-tight text-black/90 ">
                                    Requester</h5>
                                <p
                                    class="{{ $profile_qrcode_id ? '' : 'border' }} rounded w-full aspect-square flex items-center justify-center font-semibold">
                                    @if ($profile_qrcode_id)
                                        <img src="{{ $profile_qrcode_json }}" class="w-full h-full">
                                    @else
                                        Pending
                                    @endif
                                </p>
                ...
                @if ($watcher_rule == 'reciver')
                    <reciver_verification class="w-[750px] flex items-center justify-end mx-auto">
                        @if ($cert->reciver_verify == 0)
                            <a href="{{ route('cert.verify', ['id' => $cert->id, 'watcher_mod' => $watcher_rule]) }}"
                                class="js-verify-certificate bg-primary rounded text-white text-center px-4 py-2">Verify
                                this
                                certificate as reciver</a>
                        @else
                            <a disabled
                                class="js-verify-certificate bg-neutral rounded text-white text-center px-4 py-2">Allready
                                verified</a>
                        @endif
                ...
                @if ($watcher_rule == 'creator')
                    <creator_verification class="w-[750px] flex items-center justify-end mx-auto">
                        @if ($cert->creator_verify == 0)
                            <a href="{{ route('cert.verify', ['id' => $cert->id, 'watcher_mod' => $watcher_rule]) }}"
                                class="js-verify-certificate bg-primary rounded text-white text-center px-4 py-2">Verify
                                this
